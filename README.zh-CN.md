...
standalone prompts、examples 和验证脚本；真正的运行界面仍然是 Linear。
...
- [复制包](dist/zh-CN/prompts/)：一次性初始化 prompt 和 schedules 用的 loop prompts。
...
Markdown run note 约定、runtime issue 日志格式。
...
2. 把 [dist/zh-CN/prompts/](dist/zh-CN/prompts/) 里的 standalone prompt 放到
   对应运行入口：`initial-setup.standalone.md` 手动运行一次，其它 loop prompt 粘到
   AG 平台对应 schedule。

3. 手动运行 `initial-setup.standalone.md`。它会检查本地目录、Linear workflow
